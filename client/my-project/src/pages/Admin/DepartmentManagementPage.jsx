...
import { 
  Building2, Plus, Loader2, Layers, X, PlusCircle, ListFilter, Pencil, RotateCcw
} from "lucide-react";
...
export default function CreateDepartmentPage() {
  ...
  const addSpecialization = () => {
    const val = specInput.trim().toUpperCase();
    if (!val) return;
    if (form.specializations.includes(val)) return toast.error("Specialization already added");
    setForm(prev => ({ ...prev, specializations: [...prev.specializations, val] }));
    setSpecInput("");
  };

  const removeSpecialization = (index) => {
    setForm(prev => ({ ...prev, specializations: prev.specializations.filter((_, i) => i !== index) }));
  };

  const handleFormSubmit = (e) => {
    e.preventDefault();
    if (!form.department.trim()) return toast.error("Department name is required");
    if (form.specializations.length === 0) return toast.error("Add at least one specialization");
    setShowConfirm(true);
  };
  ...
  const confirmAction = async () => {
    ...
    try {
      let res;
      if (editId) {
        res = await departmentService.updateDepartment(editId, form);
      } else {
        res = await departmentService.createDepartments(form);
      }

      if (res.success || res) { // Checking for res.success or just truthy res depending on your API
        toast.success(editId ? "Department updated!" : "Department created!");
        setForm({ department: "", specializations: [] });
  ...
  const handleEditClick = (dept) => {
    ...
    setSpecInput(""); // Reset input field
    window.scrollTo({ top: 0, behavior: 'smooth' }); // Better UX to show the form
  };
 ...
 return (
    <div className="min-h-screen bg-[#F8FAFC] p-4 md:p-8 pb-20">
          ...
          <div className="lg:col-span-3 space-y-8">
            <form onSubmit={handleFormSubmit} className="space-y-8">
              <SectionCard 
                ...
                subtitle={editId ? "Modify existing data" : "Department Entry"} 
              ...
              >
                ...
                <div className="space-y-4">
                  <label className="block text-slate-700 text-sm font-bold tracking-tight">Specializations</label>
                  <div className="flex gap-2">
                    <input
                      ...
                      placeholder="Add code (e.g. CSAI)"
                      className="flex-1 bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-sm font-semibold focus:outline-none focus:border-indigo-500 transition-all"
                    />
                    <button type="button" onClick={addSpecialization} className="p-3 rounded-2xl bg-slate-900 text-white hover:bg-indigo-600 transition-all">
                      <PlusCircle size={20} />
                    </button>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {form.specializations.map((spec, i) => (
                      <span key={i} className="flex items-center gap-2 bg-indigo-50 text-indigo-600 px-3 py-1.5 rounded-xl text-[11px] font-black tracking-widest border border-indigo-100">
                        {spec}
                        <X size={14} className="cursor-pointer" onClick={() => removeSpecialization(i)} />
                      </span>
                    ))}
                  </div>
                </div>
              ...
              <div className="space-y-4 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
                {fetching ? (
                  ...
                  <div 
                    ...
                    className={`bg-white border p-5 rounded-3xl shadow-sm transition-all group cursor-pointer active:scale-95 ${editId === dept._id ? 'border-indigo-500 ring-2 ring-indigo-500/10' : 'border-slate-200 hover:border-indigo-200'}`}
                  >
                    <div className="flex justify-between items-start mb-3">
                      <h4 className="text-slate-900 font-bold text-xs leading-tight uppercase tracking-tight">{dept.department}</h4>
                      <Pencil size={14} className={`transition-colors ${editId === dept._id ? 'text-indigo-500' : 'text-slate-300'}`} />
                    </div>
                    <div className="flex flex-wrap gap-1.5">
                      {dept.specializations.map((s, idx) => (
                        <span key={idx} className="text-[9px] font-bold bg-slate-50 text-slate-400 px-2 py-0.5 rounded-md border border-slate-100 uppercase">
                          {s}
                        </span>
                      ))}
                    </div>
      ...
      <ConfirmModal
        ...
        message={editId 
          ? `You are updating "${form.department}". This change will reflect across the system.` 
          : `This will create "${form.department}" with ${form.specializations.length} specializations.`
        }
