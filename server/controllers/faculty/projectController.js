...
/**
 * @desc    Update project title, domain, or description
 * @route   PATCH /api/faculty/projects/:projectId
 * @access  Private (faculty only — must be a supervisor of the project's group)
 */
export const updateProject = async (req, res, next) => {
  try {
    const { projectId } = req.params;
    const { title, domain, description, approve } = req.body;
    ...
    if (domain !== undefined) {
      if (typeof domain !== "string" || !domain.trim()) {
        return res.status(400).json({ success: false, message: "Domain must be a non-empty string." });
      }
    ...
    if (approve !== undefined && typeof approve === "boolean") {
        ...
        else updates.status = "Proposed";
    }

    if (Object.keys(updates).length === 0) {
      return res.status(400).json({ success: false, message: "Provide at least one field to update: title, domain, or description, status" });
    }
    ...
    // ── 4. Faculty must be a supervisor of the project's group ────────────────
    const isSupervisor = await Group.exists({
      _id:         project.group,
      supervisors: req.faculty.id,   // from token — no extra Faculty fetch
    });

    if (!isSupervisor) {
      return res.status(403).json({ success: false, message: "You are not a supervisor of this project's group." });
    }
    ...
    const updated = await Project.findByIdAndUpdate(
      projectId,
      { $set: updates },
      { new: true, runValidators: true, select: "title domain description semester status" }
    ).lean();
